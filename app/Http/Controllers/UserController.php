...
public function prueba(Request $request){
    ...
    {
        try {
            $user = User::findOrFail($id); //para recuperar usuarios 
    ...
    function createUser(Request $request)
    {
        try {
            $data = $request->json()->all();
            $dataUser = $data['user'];
            $dataProfessional = $data['professional'];//localhost:8089/users clave valor 
            DB::beginTransaction(); //lo insertado debe funcionar correctamente  se utuliza cuando ahi mas de 2 transacciones

            $user = User::create([
                //orm eloquent

                'name' => strtoupper($dataUser['name']), //nombre del campo de la base de datos 
                'user_name' => $dataUser['user_name'],
                'email' => $dataUser['email'],
                'password' => Hash::make($dataUser['password']),//hash encriptar la clave
